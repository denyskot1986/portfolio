...
const translations: Record<string, Record<string, ProductTranslation>> = {
  ...
  "orban": {
    RU: {
      ...
      longDescription: "Orban — агент Finekot для ведения малого бизнеса так, чтобы в нём не утонуть. Не теряет клиентов, не забывает задач, пишет утренний бизнес-бриф и отвечает на рутинные вопросы клиентов в Telegram/WhatsApp пока ты занят настоящей работой. Под капотом говорит на MCP — открытом стандарте от Anthropic — поэтому не просто болтает про твою CRM, а открывает её и делает дело. Хостится у Finekot, с бизнес-памятью и интеграциями из коробки.",
      features: [
    ...
    UA: {
      ...
      longDescription: "Orban — агент Finekot для ведення малого бізнесу так, щоб у ньому не потонути. Не губить клієнтів, не забуває задач, пише ранковий бізнес-бриф і відповідає на рутинні запити клієнтів у Telegram/WhatsApp поки ти зайнятий справжньою роботою. Під капотом говорить на MCP — відкритому стандарті від Anthropic — тому не просто базікає про твою CRM, а відкриває її і робить справу. Хоститься у Finekot, з бізнес-пам'яттю та інтеграціями з коробки.",
      features: [
