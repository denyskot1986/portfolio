...
const translations: Record<string, Record<string, ProductTranslation>> = {
  "ilucy": {
    RU: {
      ...
      category: "Агенты",
      description: "Звонит маме или папе дважды в день. Напоминает о таблетках. Говорит про дачу, внуков, прошлое. Сразу пишет тебе, если что-то не так.",
    ...
    UA: {
      ...
      category: "Агенти",
      description: "Дзвонить мамі або татові двічі на день. Нагадує про таблетки. Говорить про дачу, онуків, минуле. Одразу пише тобі, якщо щось не так.",
  ...
  "orban": {
    RU: {
      ...
      longDescription: "Orban — агент от Finekot для ведения малого бизнеса так, чтобы в нём не утонуть. Не теряет клиентов, не забывает задач, пишет утренний бизнес-бриф и отвечает на рутинные вопросы клиентов в Telegram/WhatsApp пока ты занят настоящей работой. Под капотом говорит на MCP — открытом стандарте от Anthropic — поэтому не просто болтает про твою CRM, а открывает её и делает дело. Хостится у Finekot, с бизнес-памятью и интеграциями из коробки.",
      features: [
    ...
    UA: {
      ...
      longDescription: "Orban — агент від Finekot для ведення малого бізнесу так, щоб у ньому не потонути. Не губить клієнтів, не забуває задач, пише ранковий бізнес-бриф і відповідає на рутинні запити клієнтів у Telegram/WhatsApp поки ти зайнятий справжньою роботою. Під капотом говорить на MCP — відкритому стандарті від Anthropic — тому не просто базікає про твою CRM, а відкриває її і робить справу. Хоститься у Finekot, з бізнес-пам'яттю та інтеграціями з коробки.",
      features: [
  ...
  "idoctor": {
    RU: {
      ...
      category: "Агенты",
      description: "Загружай анализы, рецепты, заключения врачей. iDoctor помнит всё. Рекомендует по образу жизни, ловит паттерны, слушает в тяжёлые моменты.",
    ...
    UA: {
      ...
      category: "Агенти",
      description: "Завантажуй аналізи, рецепти, висновки лікарів. iDoctor пам'ятає все. Радить по способу життя, ловить патерни, слухає у важкі моменти.",
  ...
  "iada": {
    RU: {
      ...
      longDescription: "iAda — агент-исследователь Finekot, построенный как замена Perplexity для тех кто делает реальную работу: консультантов, аналитиков, основателей, журналистов. Работает в двух режимах. FAST: один вопрос, ~30–60 секунд, ответ с 5+ источниками. DEEP: многопроходное расследование по 20+ источникам, возвращает структурированный отчёт за 5–10 минут с противоречиями, уровнями достоверности и трассируемыми фактами. Помнит твои ресёрч-треды между сессиями.",
      features: [
    ...
    UA: {
      ...
      longDescription: "iAda — агент-дослідник Finekot, побудований як заміна Perplexity для тих хто робить реальну роботу: консультантів, аналітиків, засновників, журналістів. Працює у двох режимах. FAST: одне питання, ~30–60 секунд, відповідь з 5+ джерелами. DEEP: багатопрохідне розслідування по 20+ джерелах, повертає структурований звіт за 5–10 хвилин із протиріччями, рівнями достовірності та трасованими фактами. Пам'ятає твої ресерч-треди між сесіями.",
      features: [
  ...
  "ihogol": {
    RU: {
      ...
      features: [
        ...
        { title: "Разбор engagement", desc: "После публикации iHogol тянет метрики и говорит что сработало в твоём голосе — а что ушло в сторону." },
      ],
    ...
    UA: {
      ...
      features: [
        ...
        { title: "Розбір engagement", desc: "Після публікації iHogol тягне метрики і каже що спрацювало у твоєму голосі — а що пішло в сторону." },
      ],
  ...
  "iborya": {
    RU: {
      tagline: "Твой личный AI chief of staff",
      category: "Агенты",
      description: "Личный AI-секретарь, живущий в твоём Telegram. Календарь, инбокс, задачи, подготовка ко встречам — всё под контролем, голосом или текстом.",
      longDescription: "iБоря — твой личный chief of staff. AI-агент, который сидит в твоём Telegram и впитывает хаос дня. Утренний бриф, сортировка почты, подготовка и разбор встреч, голосовые заметки в задачи, приём идей пока ты идёшь или едешь. Заточен под одного человека — тебя. С персональной памятью: помнит твоих людей, проекты, обещания. Чем дольше работаешь — тем точнее.",
      features: [
        { title: "Утренний бриф", desc: "В 8:00 в Telegram: приоритеты дня, pre-read к каждой встрече, follow-up за тобой, флаги на провисающем." },
        { title: "Сортировка почты", desc: "Разбирает входящие: сейчас / сегодня / на неделю / игнор. Черновики ответов в твоём голосе — ты только жмёшь send." },
        ...
        { title: "Голос → задачи", desc: "Сбросил голосовое с 3-мя пунктами. iБоря разбил их на задачи в Todoist/Notion с дедлайнами и контекстом." },
        { title: "Персональная память", desc: "Помнит твоих людей, твои проекты, что ты обещал и когда. Чем дольше пользуешься — тем точнее." },
      ],
    ...
    UA: {
      tagline: "Твій особистий AI chief of staff",
      category: "Агенти",
      description: "Особистий AI-секретар, що живе у твоєму Telegram. Календар, інбокс, задачі, підготовка до зустрічей — усе під контролем, голосом або текстом.",
      longDescription: "iБоря — твій особистий chief of staff. AI-агент, який сидить у твоєму Telegram і вбирає хаос дня. Ранковий бриф, сортування пошти, підготовка та розбір зустрічей, голосові нотатки в задачі, прийом ідей поки ти йдеш або їдеш. Заточений під одну людину — тебе. З персональною пам'яттю: пам'ятає твоїх людей, проєкти, обіцянки. Чим довше працюєш — тим точніше.",
      features: [
        { title: "Ранковий бриф", desc: "О 8:00 у Telegram: пріоритети дня, pre-read до кожної зустрічі, follow-up за тобою, прапори на провисаючому." },
        { title: "Сортування пошти", desc: "Розбирає вхідні: зараз / сьогодні / на тиждень / ігнор. Чернетки відповідей у твоєму голосі — ти лише тиснеш send." },
        ...
        { title: "Голос → задачі", desc: "Скинув голосове з 3-ма пунктами. iБоря розбив їх на задачі у Todoist/Notion з дедлайнами та контекстом." },
        { title: "Персональна пам'ять", desc: "Пам'ятає твоїх людей, твої проєкти, що ти обіцяв і коли. Чим довше користуєшся — тим точніше." },
      ],
