...
export default function DiscoverPage() {
  ...
  const fetchNext = useCallback(async (currentHistory: HistoryItem[]) => {
    ...
    try {
      const res = await fetch("/api/discover", {
        ...
        body: JSON.stringify({ action: "next", history: currentHistory }),
      });
  ...
  const fetchAnalysis = useCallback(async (finalHistory: HistoryItem[]) => {
    setStage("analyzing");
    try {
      const res = await fetch("/api/discover", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ action: "analyze", history: finalHistory }),
      });
      const data = await res.json();
